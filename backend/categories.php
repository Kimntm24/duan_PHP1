...
<head>
  ...
  <title>Categories - Furina Shop</title>
</head>
...
<body>
  ...
  <main>
    <div class="container">
      ...
      <a href="add-category.php" class="btn btn-secondary mb-2 float-end">Add new</a>
      <table class="table table-bordered table-striped table-hover">
        <thead class="text-center">
          <th scope="col" class="py-3 fs-5">ID</th>
          <th scope="col" class="py-3 fs-5" style="width: 50%;">Name</th>
          <th scope="col" colspan="2" class="py-3 fs-5">Action</th>
          </tr>
        </thead>
        <tbody>
          <?php
          ...
          $stmt = $conn->prepare("SELECT * FROM `categories` ORDER BY `id` DESC LIMIT $start, $limit");
          $stmt->execute();
          ...
          foreach ($categories as $category) {
            echo '<tr>
                    ...
                    <td><a href="../backend/edit-category.php?id=' . $category['id'] . '" class="btn btn-warning">Edit</a></td>
                    <td><button onclick="confirmDelete(' . $category['id'] . ', \'' . $category['name'] . '\')" type="button" class="btn btn-danger">Delete</button></td>
                  </tr>';
          }
          ?>
        </tbody>
  ...
  <script>
    function confirmDelete(id, name) {
      if (confirm(`Are you sure you want to delete "${name}"`)) {
        window.location.href = `../backend/delete-category.php?id='${id}'`
      }
    }
  </script>
</body>
