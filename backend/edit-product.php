...
<head>
  ...
  <title>Edit Product - Furina Shop</title>
</head>
        ...
        if ($file['size']) {
          ...
          if ($file["error"] == 0) {
            if (!in_array($file["type"], $imageTypes)) {
              $errors['roomImage']['type'] = 'The uploaded file must be an image file (jpeg, png, gif)';
            }

            if ($file["size"] > $maxFileSize) {
              $errors['roomImage']['size'] = 'Upload file size must be less than ' . $maxFileSize / 1024 / 1024 . 'MB';
            }
          }
        }
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <select class="form-select" id="category" name="category">
            <?php
            echo '<option value="' . $product['category'] . '" hidden>' . $product['category'] . '</option>';
        ...
        <div class="mb-3">
          ...
          <img id="currentImage" class="d-block mb-2" style="height: 100px; width: 100px;" src="../uploads/<?php echo $product['image']; ?>" alt="<?php echo $product['name']; ?>">
          <input type="file" class="form-control" id="room-image" name="roomImage">
          <?php echo !empty($errors['roomImage']) ? '<p class="text-danger">' . reset($errors['roomImage']) . '</p>' : ''; ?>
        </div>
